...
mod tests {
    ...
    fn document_shows_committed_line_counts_without_changing_the_message() -> gix_testtools::Result {
        ...
        let mut tree: gix::objs::Tree = repository.find_tree(unusual.tree)?.decode()?.into();
        tree.entries.push(gix::objs::tree::Entry {
            filename: b"line\nbreak".as_bstr().into(),
            mode: gix::objs::tree::EntryKind::Blob.into(),
            oid: repository.write_blob(b"line\n")?.detach(),
        });
        tree.entries.sort();
        unusual.tree = repository.write_object(&tree)?.detach();
