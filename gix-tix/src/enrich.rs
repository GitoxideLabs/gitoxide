...
pub(crate) fn open(repo: &gix::Repository) -> Result<gix::note::Platform> {
    open_at(repo, REF_NAME)
}

pub(crate) fn open_tree(repo: &gix::Repository) -> Result<gix::note::Platform> {
    open_at(repo, TREE_REF_NAME)
}

fn open_at(repo: &gix::Repository, reference: &str) -> Result<gix::note::Platform> {
    repo.notes()
...
mod tests {
    ...
    fn checks_pass_follows_only_the_exact_tree_and_preserves_other_fields() -> gix_testtools::Result {
        ...
        let note = notes
            ...
            .into_iter()
            .next()
            .expect("the tree enrichment exists");
        let config = File::try_from(note.blob.data.as_bstr())?;
        assert_eq!(
    ...
    fn malformed_enrichments_are_not_overwritten() -> gix_testtools::Result {
        ...
        repo.notes()
            .map_err(gix::Exn::into_error)?
            .add_to_ref(reference.as_ref(), tree, b"[tree")
            .map_err(gix::Exn::into_error)?;
        assert!(
