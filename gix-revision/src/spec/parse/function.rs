...
pub fn parse(mut input: &BStr, delegate: &mut impl Delegate) -> Result<(), Exn<Error>> {
    ...
    if input.is_empty() {
        delegate
            .done()
            .or_raise(|| Error::new("delegate.done() failed after all input was consumed"))
    } else {
        Err(Error::new_with_input("unconsumed input", input).raise())
    }
}
...
fn try_set_prefix(delegate: &mut impl Delegate, hex_name: &BStr, hint: Option<delegate::PrefixHint<'_>>) -> Option<()> {
    gix_hash::Prefix::from_hex(hex_name.to_str().expect("hexadecimal only"))
        .ok()
        .and_then(|prefix| delegate.disambiguate_prefix(prefix, hint).ok())
}
        ...
        (consecutive_hex_chars.unwrap_or(0) >= gix_hash::Prefix::MIN_HEX_LEN)
            ...
            .or_else(|| {
                name.is_empty().then_some(()).or_else(|| {
                    ...
                    {
                        let res = delegate.find_ref(name).ok()?;
                        has_ref_or_implied_name = true;
                        res.into()
                    }
                })
            })
            .ok_or_else(|| Error::new_with_input("couldn't parse revision", input))?;
    }
            ...
            b'^' => {
                    ...
                    .transpose()?
                {
                    if negative {
                        ...
                        delegate
                            .done()
                            .or_raise(|| Error::new_with_input("navigation succeeded, delegate.done() failed", rest))?;
                        return Ok(rest);
                    } else if number == 0 {
