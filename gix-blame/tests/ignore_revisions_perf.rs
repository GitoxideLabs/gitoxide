...
fn simple_performance_test() -> gix_testtools::Result<()> {
    ...
    // Calculate averages
    let avg_without = durations_without.iter().map(std::time::Duration::as_nanos).sum::<u128>() / durations_without.len() as u128;
    let avg_with_empty = durations_with_empty.iter().map(std::time::Duration::as_nanos).sum::<u128>() / durations_with_empty.len() as u128;
    
    println!("Performance comparison (averaged over {} runs):", durations_without.len());
    println!("Without ignore revisions: {:?}", std::time::Duration::from_nanos(avg_without as u64));
    println!("With empty ignore revisions: {:?}", std::time::Duration::from_nanos(avg_with_empty as u64));
    ...
    assert!(overhead.abs() < 20.0, "Overhead is outside reasonable bounds: {overhead:.2}%");
    ...
    assert!(overhead_ratio > 0.8 && overhead_ratio < 1.2, "Performance difference is too large: {:.2}%", (overhead_ratio - 1.0) * 100.0);
