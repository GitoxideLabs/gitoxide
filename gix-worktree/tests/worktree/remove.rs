...
fn administrative_data_is_removed_after_checkout_removal_fails() -> crate::Result {
    let tmp = gix_testtools::tempfile::tempdir()?;
    // Invalid filesystem paths fail even when CI runs as root and can bypass permissions.
    let work_dir = tmp.path().join("linked\0invalid");
    let git_dir = tmp.path().join("repo.git/worktrees/linked");
    fs::create_dir_all(&git_dir)?;
    fs::write(git_dir.join("HEAD"), b"ref: refs/heads/topic\n")?;

    let err = gix_worktree::remove::remove(&work_dir, &git_dir, gix_features::progress::Discard)
        .expect_err("an invalid checkout path cannot be removed");
