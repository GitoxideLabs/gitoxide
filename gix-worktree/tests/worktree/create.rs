...
fn prepares_git_compatible_links_and_unique_sanitized_names() -> crate::Result {
    ...
    assert_eq!(
        ...
        format!(
            "{}\n",
            gix_path::to_unix_separators_on_windows(gix_path::into_bstr(first.work_dir().join(".git")))
        )
    );
    assert_eq!(
        ...
        format!(
            "gitdir: {}\n",
            gix_path::to_unix_separators_on_windows(gix_path::into_bstr(first.git_dir()))
        )
    );
