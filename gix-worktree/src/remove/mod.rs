...
/// Traversal skips optional metadata, except for directory checks that protect Unix mount boundaries.
...
fn remove_root_with_after_scan(
    ...
    let descend = move |entry: &dua_core::Entry| {
        ...
        may_descend(containing_device, root_is_filesystem_root, entry).unwrap_or(false)
    };
    #[cfg(not(unix))]
            ...
            match entry {
                Ok(entry) => {
                    ...
                    if entry.file_type.is_dir() {
                        #[cfg(unix)]
                        match may_descend(containing_device, root_is_filesystem_root, &entry) {
                            Ok(true) => {}
