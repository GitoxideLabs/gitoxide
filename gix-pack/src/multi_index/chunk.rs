...
pub mod index_names {
    ...
    pub fn from_bytes(mut chunk: &[u8], num_packs: u32, alloc_limit_bytes: Option<usize>) -> Result<Vec<PathBuf>> {
        ...
        if num_packs > chunk.len() {
            return Err(gix_error::corruption("Pack count exceeds the available pack-name data").raise());
        }
        ...
        for _ in 0..num_packs {
            ...
            if let Some(previous) = out.last()
                && previous >= &path
            {
                return Err(gix_error::corruption("The pack names were not ordered alphabetically").raise());
            }
            out.push(path);
        ...
        if !chunk.is_empty() && !chunk.iter().all(|b| *b == 0) {
            return Err(gix_error::corruption("Non-padding bytes found after all paths were read").raise());
        }
