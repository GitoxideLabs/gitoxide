...
use crate::{
    find::Header,
    store::{find::DeltaBaseRecursion, handle, load_index},
};
    ...
    pub(crate) fn try_header_inner<'b>(
        ...
        if let Some(r) = recursion {
            if r.depth >= self.max_recursion_depth {
                return Err(Metadata::new("Reached recursion limit while resolving ref delta bases")
                    .with("max_depth", self.max_recursion_depth)
                    .with("object_id", r.original_id.to_string())
                    .raise_erased());
            }
