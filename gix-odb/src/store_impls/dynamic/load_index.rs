...
/// Expose the original error as a source so its metadata survives sharing.
#[derive(Debug)]
struct SharedRefresh(Arc<gix_error::Error>);

impl std::fmt::Display for SharedRefresh {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        std::fmt::Display::fmt(self.0.as_ref(), f)
    }
}

impl std::error::Error for SharedRefresh {
...
impl super::Store {
    ...
    fn load_next_index(&self, mut catalog: Arc<Catalog>) -> ExnResult<bool> {
    ...
    fn retry_failed_index(&self, catalog: &Catalog) -> ExnResult<bool> {
        let index = &catalog.index;
        let claimed = index.next_index_to_load.load(Ordering::SeqCst);
        for &slot_id in index.slot_indices.iter().take(claimed) {
            let slot = &catalog.slots[slot_id];
            // A claimed slot still publishes Unloaded while its loader holds the lock.
            if !Option::as_ref(&slot.files.load()).is_some_and(IndexAndPacks::index_is_failed) {
                continue;
            }
            #[cfg(feature = "test-support")]
            ...
            return res.map(|_| true).or_erased();
        }
        Ok(false)
    }
    ...
    /// (unsigned additional slots), or `limit` (unsigned maximum generation).
    pub(crate) fn consolidate_with_disk_state(
        ...
        if completed_refreshes != current_completed_refreshes {
            ...
            if let Some((generation, error)) = last_error.as_ref()
                && *generation == current_completed_refreshes
            {
                return Err(SharedRefresh(Arc::clone(error)).raise_erased());
            }
        ...
        if num_indices_needed > slot_limit {
            ...
            return self.finish_refresh(
                ...
                Err(Message::new("The object database has too few index slots")
                    .with("current", slots.len())
                    .with("needed", num_indices_needed - slots.len())
                    .raise_erased()),
            );
        }
        ...
        let generation = if needs_generation_change {
            match index.generation.checked_add(1) {
                ...
                None => {
                    ...
                    return self.finish_refresh(
                        current_completed_refreshes,
                        Err(Message::new("Cannot advance the object database generation")
        ...
        let snapshot = if index.state_id() == new_index.state_id() {
            ...
            if load_new_index && let Err(err) = self.load_next_index(new_catalog) {
    ...
    fn finish_refresh<T>(&self, previous_completion: usize, result: ExnResult<T>) -> ExnResult<T> {
        ...
        match result {
            ...
            Err(err) => {
                let err = Arc::new(err.into_error());
                *self.last_disk_state_consolidation_error.lock() = Some((completion, Arc::clone(&err)));
                ...
                Err(SharedRefresh(err).raise_erased())
            }
        }
    }

    /// Read failures include [metadata](gix_error::Exn::metadata()) `path` (native directory or index path).
    ...
    /// Returns whether the index was left loaded and whether its file needs another load attempt.
    fn assure_slot_matches_index(
        ...
        let (outcome, changed_slot) = if bundle.index_is_missing() || bundle.index_is_failed() {
            if bundle.index_is_failed() && bundle.mtime() == mtime {
...
mod tests {
    ...
    #[test]
    fn shared_refresh_preserves_error_details() {
        let original = Arc::new(
            gix_error::retryable("refresh failed")
                .with("current", 1usize)
                .raise()
                .into_error(),
        );
        let shared = SharedRefresh(Arc::clone(&original)).raise_erased();
        assert!(
            shared.is_retryable(),
            "waiting handles retain the original classification"
        );
        assert_eq!(
            shared.metadata().collect::<Vec<_>>(),
            original.metadata().collect::<Vec<_>>(),
            "waiting handles retain the complete error metadata"
        );
    }
