...
pub(crate) mod index_lookup {
    ...
    impl handle::IndexLookup {
        ...
        pub(crate) fn pack(&self, pack_id: gix_pack::data::Id) -> Option<&Arc<gix_pack::data::File>> {
            match &self.file {
        ...
        pub(crate) fn same_index(&self, other: &Self) -> bool {
            match (&self.file, &other.file) {
                (
                    handle::SingleOrMultiIndex::Single { index: left, .. },
                    handle::SingleOrMultiIndex::Single { index: right, .. },
                ) => Arc::ptr_eq(left, right),
                (
                    handle::SingleOrMultiIndex::Multi { index: left, .. },
                    handle::SingleOrMultiIndex::Multi { index: right, .. },
                ) => Arc::ptr_eq(left, right),
                _ => false,
            }
        }
    ...
    S: Deref<Target = super::Store> + Clone,
{
    ...
    /// Clones share retained locations, including those discovered after cloning.
            ...
            crate::store::init::Options {
                slots: match s.slot_limit {
                    ...
                    None => crate::store::init::Slots::Growable {
                        initial: catalog.slots.len().try_into().unwrap_or(u16::MAX),
                    },
                },
    ...
    S: Deref<Target = super::Store> + Clone,
{
    fn clone(&self) -> Self {
        super::Handle {
            ...
            retained_indices: Arc::clone(&self.retained_indices),
