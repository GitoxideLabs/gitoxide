...
use gix_error::{Class, ErrorExt, Exn};
...
impl Machine {
    ...
    fn execute(&mut self, instruction: &[u8]) -> (Opcode, Outcome) {
        ...
        // Error classes describe causes, not the failed operation's stage. Check preservation for every failure.
        if matches!(outcome, Outcome::Query(Query::Error(_)) | Outcome::WriteError(_)) {
    ...
    fn iterate_store(&self, handle_index: usize, ordering: usize) -> Outcome {
        ...
        let iter = match handle.value.iter() {
            Ok(iter) => iter,
        ...
        for id in iter.take(16) {
            let id = match id {
                Ok(id) => id,
                Err(err) => return query_error(err.raise_erased()),
            };
            assert!(
        ...
        for (index, handle) in self.handles.iter().enumerate().filter_map(|(index, handle)| {
            ...
            if let Some(id) = handle.observed_packed_object.filter(|id| reachable.contains(id)) {
                assert_eq!(
                    ...
                    Outcome::Query(Query::Found),
                    "a failed operation through handle {selected_index} preserves {id} observed by shared-store handle {index}"
                );
            }
        }
    }
...
fn query_error(err: Exn) -> Outcome {
    Outcome::Query(Query::Error(error_class(err)))
}
...
fn write_error(err: Exn) -> Outcome {
    Outcome::WriteError(error_class(err))
}

fn error_class(err: Exn) -> Option<Class> {
    if trace_enabled() {
        eprintln!("{err:#}");
    }
    err.classify().next().map(|classification| classification.class())
}
