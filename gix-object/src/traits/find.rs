...
mod ext {
    ...
    fn forward(err: find::Error) -> gix_error::Exn {
        gix_error::Error::from_boxed(err).raise_erased()
    }
    ...
    pub trait HeaderExt: super::Header {
        ...
        fn header(&self, id: &gix_hash::oid) -> Result<crate::Header, find::existing::Error> {
            self.try_header(id).map_err(forward)?.ok_or_else(|| not_found(id))
        }
    }
    ...
    pub trait FindExt: super::Find {
        ...
        fn find<'a>(
            ...
            self.try_find(id, buffer).map_err(forward)?.ok_or_else(|| not_found(id))
        }
