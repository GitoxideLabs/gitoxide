        ...
        match content_buf {
            ...
            Some(content) => Ok(Some(
                loose::Reference::try_from_path(full_name.to_owned(), &content, self.object_hash)
                    ...
                    .or_raise_erased(|| ReferenceCreation {
...
#[derive(Debug)]
pub struct ReferenceCreation {
    /// The resolved reference path, relative to the Git directory.
    pub relative_path: PathBuf,
}

impl std::fmt::Display for ReferenceCreation {
    #[allow(clippy::unnecessary_debug_formatting)]
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        write!(f, "The reference at {:?} could not be instantiated", self.relative_path)
    }
}

impl std::error::Error for ReferenceCreation {}
