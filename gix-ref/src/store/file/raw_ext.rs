...
use gix_error::{CorruptionError, ErrorExt, Exn, Metadata, NotFoundError, ResultExt, message};
...
pub trait ReferenceExt: Sealed {
    ...
    fn peel_to_id_in_place(&mut self, store: &file::Store, objects: &dyn gix_object::Find) -> Result<ObjectId, Exn>;
    ...
    /// non-symbolic object.
    fn peel_to_id(&mut self, store: &file::Store, objects: &dyn gix_object::Find) -> Result<ObjectId, Exn>;
    ...
    /// Like [`ReferenceExt::follow()`], but follows all symbolic references while gracefully handling loops,
    /// altering this instance in place.
    /// Cycle failures include metadata `path` (native path); depth-limit failures include `max_depth` (unsigned integer).
    ...
    fn follow(&self, store: &file::Store) -> Option<Result<Reference, Exn>>;

    /// Follow this symbolic reference one level and return the ref it refers to,
    /// possibly providing access to `packed` references for lookup if it contains the referent.
    ///
    /// Returns `None` if this is not a symbolic reference, hence the leaf of the chain.
    fn follow_packed(&self, store: &file::Store, packed: Option<&packed::Buffer>) -> Option<Result<Reference, Exn>>;
}
            ...
            None => {
                ...
                let peeled_id = loop {
                    let gix_object::Data {
                        ...
                        .and_then(|object| {
                            object.ok_or_else(|| NotFoundError::new("Object could not be found").raise_erased())
                        })
                        .or_raise_erased(|| {
                            Metadata::new("Could not peel reference to an object")
            ...
            Target::Symbolic(_) => {
                ...
                while let Some(next) = cursor.follow_packed(store, packed) {
                    ...
                    if seen.contains(&next.name) {
                        return Err(CorruptionError::new("Symbolic references form a cycle")
                            .and_raise(
                                Metadata::new("Aborting reference cycle")
                    ...
                    *cursor = next;
                    ...
                    if seen.len() == MAX_REF_DEPTH {
                        return Err(Metadata::new("Symbolic reference depth limit exceeded")
