//! Benchmark creating, packing, and reading one Git note per commit in an existing commit-graph.
//!
//! Run from the workspace root, selecting a repository with a commit-graph and an unused notes ref:
//!
//! ```sh
...
//! ```
//!
//! Replace `all` with a positive limit for a smaller run. Each note contains the annotated commit's
//! hexadecimal ID followed by a newline. One retained notes state stages all edits before serializing
...
fn main() -> Result<()> {
    ...
    if repository.as_os_str() == "--help" {
        println!("Usage: gix-notes-bench [repository=.] [limit=all] [ref={NOTES_REF}]");
        println!("Annotate every commit in the existing commit-graph; refuse an existing notes ref.");
        return Ok(());
    }
    let limit = args
        .next()
    ...
    let mut commit_ids: Vec<_> = graph.iter_ids().map(ToOwned::to_owned).collect();
    // Graph files are OID-sorted. Sorting by the suffix spreads edits over the notes fanout.
    commit_ids.sort_unstable_by(|a, b| a.as_bytes()[8..].cmp(&b.as_bytes()[8..]));
    commit_ids.truncate(limit);
    if commit_ids.is_empty() {
        return Err("the commit-graph is empty".into());
    }
    let count = commit_ids.len();
    println!("repository: {}", repo.git_dir().display());
    println!("commits: {count} (graph contains {})", graph.num_commits());
    ...
    for (index, &commit_id) in commit_ids.iter().enumerate() {
        ...
        assert!(previous.is_none(), "each graph commit must be annotated exactly once");
        if (index + 1).is_multiple_of(10_000) && last_report.elapsed() >= Duration::from_secs(5) {
            eprintln!(
    ...
    let notes_commit_id = objects.write(&gix::objs::Commit {
        ...
        committer: signature,
        encoding: None,
        message: format!("Attach benchmark notes to {count} commit-graph commits\n").into(),
