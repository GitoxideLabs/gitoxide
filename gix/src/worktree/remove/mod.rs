...
fn resolve<'repo>(repo: &'repo crate::Repository, target: &Path) -> Result<Target<'repo>, Error> {
    ...
    let resolved_target = resolve_path(target).map_err(|source| Error::ResolvePath {
        path: target.to_owned(),
        source,
    })?;
    let mut exact_matches = Vec::new();
    if let Some(path) = main {
        let resolved = resolve_path(&path).map_err(|source| Error::ResolvePath {
            path: path.clone(),
            source,
        })?;
    ...
    for candidate in linked {
        let resolved = resolve_path(&candidate.base).map_err(|source| Error::ResolvePath {
...
fn validate_backlink(work_dir: &Path, git_dir: &Path, ignore_case: bool) -> Result<(), Error> {
    ...
    let actual = resolve_path(&actual).map_err(|source| Error::ResolveGitDir {
        path: actual.clone(),
        source,
    })?;
    let expected = resolve_path(git_dir).map_err(|source| Error::ResolveGitDir {
...
fn resolve_path(path: &Path) -> Result<std::path::PathBuf, gix_path::realpath::Error> {
    let path = gix_path::realpath(path)?;
    // Git expands Windows short names, and Rust canonical paths use verbatim prefixes.
    // Preserve missing suffixes so an inaccessible checkout can still be unregistered.
    #[cfg(windows)]
    for ancestor in path.ancestors() {
        if let Ok(canonical) = ancestor.canonicalize() {
            return Ok(canonical.join(path.strip_prefix(ancestor).expect("ancestor belongs to this path")));
        }
    }
    Ok(path)
}
