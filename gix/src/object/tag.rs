...
impl<'repo> Tag<'repo> {
    ...
    /// more than once.
    pub fn decode(&self) -> Result<gix_object::TagRef<'_>, gix_error::ValidationError> {
        gix_object::TagRef::from_bytes(&self.data, self.id.kind())
    }

    /// Decode this tag partially and return the id of its target.
    pub fn target_id(&self) -> Result<crate::Id<'repo>, gix_error::ValidationError> {
    ...
    pub fn tagger(&self) -> Result<Option<gix_actor::SignatureRef<'_>>, gix_error::ValidationError> {
