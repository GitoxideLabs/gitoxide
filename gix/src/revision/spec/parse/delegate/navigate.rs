...
impl delegate::Navigate for Delegate<'_> {
    fn traverse(&mut self, kind: Traversal) -> Result<(), Exn> {
        ...
        let objs = match self.objs[self.idx].as_mut() {
            ...
            None => {
                bail!(
                    message!("Tried to navigate the commit-graph without providing an anchor first").raise_something()
                )
            }
        };
        let repo = self.repo;
        ...
        for obj in objs.iter() {
            match kind {
                Traversal::NthParent(num) => {
                    match self.repo.find_object(*obj).or_something().and_then(|obj| {
    ...
    fn peel_until(&mut self, kind: PeelTo<'_>) -> Result<(), Exn> {
        ...
        let objs = self.objs[self.idx]
            .as_mut()
            .ok_or_raise_something(|| message!("Couldn't get object at internal index {idx}", idx = self.idx))?;
        let repo = self.repo;
        ...
        match kind {
            PeelTo::ValidObject => {
                for obj in objs.iter() {
                    if let Err(err) = repo.find_object(*obj) {
                        errors.push((*obj, err.raise_something()));
                    }
                }
            }
            ...
            PeelTo::Path(path) => {
                let lookup_path = |obj: &ObjectId| {
                    ...
                    let mut tree = repo.find_object(tree_id).or_something()?.into_tree();
                    let entry = tree
                        .peel_to_entry_by_path(gix_path::from_bstr(path))
                        .or_something()?
                        .ok_or_raise_something(|| {
                            message!(
            ...
            PeelTo::RecursiveTagObject => {
                for oid in objs.iter() {
                    match oid.attach(repo).object().and_then(Object::peel_tags_to_end) {
                        ...
                        Err(err) => errors.push((*oid, err.raise_something())),
                    }
                }
            }
        }
    ...
    fn find(&mut self, regex: &BStr, negated: bool) -> Result<(), Exn> {
        ...
        match self.objs[self.idx].as_mut() {
                        ...
                        .all()
                    {
                        Ok(iter) => {
                            ...
                            let commits = iter.map(|res| {
                                res.map_err(|err| err.raise_something()).and_then(|commit| {
                                    commit
                                        .id()
                                        .object()
                                        .map_err(|err| err.raise_something())
            ...
            None => {
                let references = self.repo.references().or_something()?;
                let references = references.all().or_something()?;
                let iter = self
                    .repo
                    .rev_walk(
                        references
                            .peeled()
                            .or_raise_something(|| message!("Couldn't configure iterator for peeling"))?
                ...
                for commit in commits {
                    ...
                    match commit {
                        ...
                        Err(err) => self.delayed_errors.push(gix_error::Error::from(err).into()),
                    }
                }
                if matched {
                    Ok(())
    ...
    fn index_lookup(&mut self, path: &BStr, stage: u8) -> Result<(), Exn> {
        ...
        self.unset_disambiguate_call();
        let index = self.repo.index().or_something()?;
        match index.entry_by_path_and_stage(path, stage) {
            ...
            None => {
                ...
                Err(message!(
                    ...
                    exists = exists
                        .then(|| ". It exists on disk")
                        .unwrap_or(". It does not exist on disk"),
