...
pub(crate) fn from_delayed_errors(errors: Vec<Exn>) -> Option<Exn> {
    (!errors.is_empty()).then(|| {
        if errors.len() == 1 {
            errors.into_iter().next().expect("it's one")
        } else {
            Exn::from_iter(errors, message!("one or more delayed errors")).erased()
        }
    })
}
