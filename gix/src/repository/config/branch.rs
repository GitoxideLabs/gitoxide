            ...
            remote::Direction::Push => {
                ...
                if remote.push_specs.is_empty() {
                    ...
                    matching_remote(name, remote.push_specs.iter(), self.object_hash())
                        .map(|res| -> Result<_, gix_error::Error> { Ok(res.or_erased()?) })
                }
            }
        }
    }
...
fn source_ref_to_full_name(source: gix_refspec::match_group::SourceRef<'_>) -> Result<FullName, gix_error::Error> {
    match source {
        gix_refspec::match_group::SourceRef::FullName(name) => {
            gix_ref::FullName::try_from(name.into_owned()).map_err(|err| {
                gix_error::Error::from(
                    err.and_raise(gix_error::ValidationError::new("The upstream branch name is invalid")),
                )
            })
        }
