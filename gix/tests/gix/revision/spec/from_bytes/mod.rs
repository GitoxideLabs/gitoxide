...
mod index {
    ...
    fn at_stage() {
        ...
        assert_eq!(
            parse_spec(":5:file", &repo).unwrap_err().leaf().to_string(),
        ...
        assert_eq!(
            parse_spec(":foo", &repo).unwrap_err().leaf().to_string(),
...
fn invalid_head() {
    ...
    // object was resolved.
    insta::assert_debug_snapshot!(err, @r#"
    No revision was produced after all input was consumed
    |
    └─ Encountered unprocessed errors when finishing
    |
    └─ Could not peel 'HEAD' to obtain its target
    |
    └─ Could not follow a single level of a symbolic reference
