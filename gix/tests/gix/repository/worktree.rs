...
mod create {
    ...
    fn attached_and_detached_worktrees_are_checked_out_and_recognized_by_git() -> crate::Result {
        ...
        assert!(
            listing.contains(gix_path::to_unix_separators_on_windows(gix_path::into_bstr(&attached_path)).to_str()?),
            "Git recognizes the attached worktree"
        );
        assert!(
            listing.contains(gix_path::to_unix_separators_on_windows(gix_path::into_bstr(&detached_path)).to_str()?),
...
mod remove {
    ...
    #[test]
    fn git_created_worktrees_resolve_native_and_canonical_paths_even_when_missing() -> crate::Result {
        let (repo, _fixture) = crate::basic_rw_repo()?;
        let workdir = repo.workdir().expect("the fixture has a worktree");
        for (name, missing, canonical) in [
            ("native", false, false),
            ("canonical", false, true),
            ("missing-native", true, false),
            ("missing-canonical", true, true),
        ] {
            let destination = workdir.join(name);
            gix_testtools::git(workdir, &format!("worktree add --detach {name} HEAD"))?;
            // On Windows this resolves short-name aliases and adds the verbatim prefix that
            // Git's registration does not use. Keep the same spelling after deleting a checkout.
            let target_path = if canonical {
            ...
            let target = repo.prepare_remove_worktree(&target_path)?;
            let private_git_dir = target.repository()?.git_dir().to_owned();
            target.remove(Force::Never, gix::progress::Discard)?;
            assert!(!destination.exists(), "the selected {name} checkout is absent");
            assert!(!private_git_dir.exists(), "the selected {name} registration is removed");
        }
        let main = repo.workdir().expect("the fixture has a worktree").canonicalize()?;
        assert!(
            matches!(
                repo.prepare_remove_worktree(&main),
                Err(gix::worktree::remove::Error::MainWorktree { .. })
            ),
            "a canonical alias cannot bypass protection of the main worktree"
        );
        Ok(())
    }
}
