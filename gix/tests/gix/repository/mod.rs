...
fn size_in_memory() {
    ...
    // Allow the selected index path, network-client cache, and refresh policy stored in both the ODB handle
    // and configuration cache. The largest observed layout is on 64-bit Windows.
    let limit = 1528;
    assert!(
        actual_size <= limit,
