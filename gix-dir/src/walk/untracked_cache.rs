...
impl<'a> Validated<'a> {
    ...
    pub(crate) fn is_dir_valid(&self, idx: usize, absolute_dir: &Path) -> bool {
        ...
        match dir.exclude_file_oid() {
            Some(expected_oid) => {
                hash_file_as_blob(&ignore_path, self.object_hash).is_some_and(|actual| actual == *expected_oid)
            }
            None => !ignore_path.exists(),
        }
    }
}
...
pub(crate) fn validate<'a>(
    ...
    if !cache_is_applicable(opts, ctx)? {
        return None;
    }
    ...
    let excludes_file = ignore
        ...
        .find(|path| {
            gix_path::realpath(*path).ok().as_deref() != Some(info_exclude_path.as_path())
        });
...
fn cache_is_applicable(opts: Options<'_>, ctx: &Context<'_>) -> Option<bool> {
    if opts.emit_ignored.is_some()
        ...
        || ctx.explicit_traversal_root.is_some()
...
fn hash_file_as_blob(path: &Path, object_hash: gix_index::hash::Kind) -> Option<gix_index::hash::ObjectId> {
    let data = std::fs::read(path).ok()?;
    gix_object::compute_hash(object_hash, gix_object::Kind::Blob, &data).ok()
}
...
mod tests {
    ...
    fn expected_ident_resolves_symlinks() -> Result<(), Box<dyn std::error::Error>> {
        ...
        assert_eq!(ident_real, ident_symlink, "identifiers must be identical for the same physical location");
