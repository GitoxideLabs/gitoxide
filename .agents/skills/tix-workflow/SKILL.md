...
Tix displays a Git hash followed by a change ID. Prefer the change ID for later travel because hashes change during edits and replay. Resolve ambiguous IDs instead of guessing. Check installed `--help`: Tix can be newer than this checkout's [specification](../../../gix-tix/spec.md).

Identify the introducing commit from the failure, tested revision, diff, and history. CI can refer to an older PR head. For ancestor edits, travel to the target and refresh `tix show` before classifying it: pending replay can temporarily hide review marks. At the intended `HEAD`, inspect there without unnecessary travel.

| Task | Default action |
| --- | --- |
| Fix a commit with `✨` | Preserve it; insert a `fixup!` immediately above it. |
| Fix a commit without `✨` | Amend it. |
...
Edits can advance refs while descendants remain pending. After this task travels for an ancestor edit, use `tix travel "$return_branch"` to replay its ancestry and restore attachment. If initially detached, return through the saved change ID or pin, not a stale hash. Honor subsequent user checkout changes: direct-target rewording does not require traveling back merely because an ancestor was rewritten. Non-HEAD edits and detached travel may need a covering current-worktree pin; inspect pins rather than manually moving refs.
