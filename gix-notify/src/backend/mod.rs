...
impl Backend {
    ...
    pub(crate) fn replace(&mut self, watches: &[Watch]) -> Result<(), Error> {
        // In particular, macOS may accept /var/... but emit /private/var/...; notify's
        // own event filter needs the registered paths to use the same physical spelling.
        ...
        for watch in &watches {
            ...
            #[cfg(target_os = "macos")]
            if watch.path.to_str().is_none() {
                // The compatibility backend would panic on such roots. Native backends need not
                // impose this limitation; return an error so callers can fall back to scanning.
                return Err(message("the macOS compatibility watcher requires a UTF-8 root path").raise());
            }
            self.watcher
                .watch(
                    &watch.path,
