...
use gix_error::{Message, OptionExt, message};
...
impl Index {
    ...
    pub fn data_by_id<'a>(&self, data: &'a [u8], kind: Id) -> Result<&'a [u8]> {
        let offset = self.offset_by_id(kind)?;
        Ok(&data[crate::range::into_usize(offset)
            .ok_or_raise(|| message("The offsets into the file couldn't be represented by usize"))?])
    }
